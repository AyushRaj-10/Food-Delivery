...
export const AdminProvider = ({ children }) => {
  ...
  const UpdateFood = async (id, payload) => {
    try {
      setLoading(true);
      setError(null);
      const response = await axios.put(`${url}/food/${id}`, payload);
      return response.data;
    } catch (err) {
      console.error("❌ UpdateFood error:", err.response?.data || err.message);
  ...
  const DeleteFood = async (id) => {
    try {
      ...
      console.error("❌ DeleteFood error:", err.response?.data || err.message);
  ...
  return (
    <AdminContext.Provider value={{ SaveFood, GetFood, UpdateFood, DeleteFood, foods, loading, error }}>
      {children}
